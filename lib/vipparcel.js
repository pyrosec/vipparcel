...
class VIPParcel {
    ...
    async _call(uri, config) {
        ...
        if (config.method === 'GET')
            uri += "?" + querystring_1.default.stringify({ authToken: this.authToken });
        else {
    ...
    async ipinfo() {
        return await (await (0, node_fetch_1.default)('https://ipinfo.io/json', { agent: this._makeAgent(), method: 'GET' })).json();
    }
